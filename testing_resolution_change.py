...
def pokemon_team_menu(manager, pokemon_player, pokemon_opponent):
    ...
    while options_running:
        ...
        utils.draw_text(surface, "Pokemon Team", font, "#b68f40", res_screen_bottom[0] // 2 - 200, res_screen_bottom[1] // 2)
    ...
    ui_battle.refresh_screen(surface, pokemon_player, pokemon_opponent)
    manager.update()
...
def bag_menu(manager, pokemon_player, pokemon_opponent):
    ...
    while options_running:
        ...
        utils.draw_text(surface, "Sac", font, "#b68f40", res_screen_bottom[0] // 2 - 200, res_screen_bottom[1] // 2)
...
def ko(manager, pokemon_player, pokemon_opponent):
    ...
    while ko_running:
        ...
        if state == 0 and elapsed >= 0.5:
            utils.print_log_ingame(surface, f"{pokemon_ko.name} {'ennemi' if pokemon_ko is pokemon_opponent else 'allié'} est KO")
            state = 1; elapsed = 0

        elif state == 1 and elapsed >= 2:
            pokemon_ko.animate_death(surface, front_or_back)
            state = 2; elapsed = 0

        elif state == 2 and elapsed >= 2:
            if pokemon_ko is pokemon_player:
                new_pokemon = trainer.send_next(surface, "back")
                if new_pokemon is None:
                    return new_pokemon, pokemon_opponent, False
                pokemon_player = new_pokemon
                utils.print_log_ingame(surface, f"{pokemon_player.name} est envoyé par {trainer.name} !")
                ui_battle.refresh_player_side(surface, pokemon_player)
            else:
                new_pokemon = opponent.send_next(surface, "front")
                if new_pokemon is None:
                    return pokemon_player, pokemon_opponent, False
                pokemon_opponent = new_pokemon
                utils.print_log_ingame(surface, f"{pokemon_opponent.name} est envoyé par {opponent.name} !")
...
def attack_menu(manager, pokemon_player, pokemon_opponent):
    ...
    while turn_running:
        ...
        moves_button = [ui_battle.draw_move(surface, move, coord[0], coord[1]) for move, coord in zip(moves_available, list_coord)]
            ...
            for i, button in enumerate(moves_button):
                if button.handle_event(event):
                    pokemon_player, pokemon_opponent, battle_state = \
                        utils.start_turn(surface, pokemon_player, pokemon_opponent, moves_available, i)
...
def battle_menu(manager):
    ...
    pokemon_player, pokemon_opponent, _ = pokemon_battle.start_battle(manager.get_surface(), trainer, opponent, background=bg_path)
    elapsed = 0
    state = ""
    showing_menu = True
    while battle_running:
        ...
        if showing_menu and state not in ["ko", "end"]:
            surface.blit(BACKGROUND_IMAGE_BOTTOM, (res_screen_bottom[0] - BACKGROUND_IMAGE_BOTTOM.get_width(),
                                                   res_screen_bottom[1] + black_band_res[1]))
            utils.print_log_ingame(surface, f"Que dois faire {pokemon_player.name} ?")
            ...
            utils.print_log_ingame(surface, f"{winner.name} a vaincu {loser.name} !", reset=True)
            elapsed = 0; state = "end"
...
def options_menu(manager):
    ...
    while options_running:
        ...
        utils.draw_text(surface, "OPTIONS", font, "#b68f40", res_screen_bottom[0] // 2 + 200, res_screen_bottom[1] // 2)
        for button in [BACK_BUTTON]:
            button.draw(surface)
